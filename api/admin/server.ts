...
async function aiExpandEmoji(count: number, existing: any[]): Promise<any[]> {
  const avoid = existing.slice(-15).map((i: any) => `${i.emoji}=${i.answer}`).join("、");
  const prompt = `你是表情包猜词游戏的题目生成器。请生成 ${count} 道题目，类别在"成语"和"影视"间自由选择。
...
【好题示例 —— 学习这种逻辑】
  ✅ 成语 🌳🐰👴⏳ → 守株待兔（树+兔+老农+等待，组合出故事场景）
  ✅ 成语 🐍✏️🦶 → 画蛇添足（蛇+画笔+脚，组合出动作）
  ✅ 成语 🍷🏹🐍 → 杯弓蛇影（杯+弓+蛇影，组合出场景）
  ✅ 影视 🐒🐷🐴👨‍🦲 → 西游记（猴+猪+马+和尚=师徒四人，用角色组合联想作品）
  ✅ 影视 🌍🚀☀️💥 → 流浪地球（地球+火箭+太阳+爆炸，用剧情元素联想）
  ✅ 影视 🚢💔🧊 → 泰坦尼克号（船+心碎+冰山，用关键情节联想）
  ✅ 影视 👑🐱 → 狮子王（皇冠+猫暗示"王+狮"，用暗喻而非直接画狮子戴冠）
  ❌ 太直白：🦁👑=狮子王（直接把答案画出来，玩家不用思考）
  ❌ 太直白：用数字 emoji 拼成语字面（如 9️⃣🐂1️⃣🦌 拼"九牛一毛"——禁止数字）

【题目范围 —— 只允许两类，违反即作废】
1. 成语：耳熟能详的中文成语（如画蛇添足、守株待兔、亡羊补牢、井底之蛙、对牛弹琴等）
2. 影视：大众熟知的电影/电视剧/动画（如西游记、泰坦尼克号、流浪地球、哈利波特、冰雪奇缘等）
绝不能生成普通名词（如"披萨""熊猫""生日"）或自造词。答案必须用最常见的说法，不要带副标题。

【emoji 硬性要求 —— 违反即作废】
1. emoji 字段只能包含 emoji 字符和连接符（+、-、空格），绝对禁止任何汉字、英文字母、数字、标点。
   ❌ 错误：🐲腾🐯跃（含汉字）、1️⃣🐎当先（含数字）、Snowman（含字母）
   ✅ 正确：🐍✏️🦶、🚢💔🧊、🐒🐷🐴👨‍🦲
2. 每题 2-6 个 emoji，必须是真实存在的 emoji（不要杜撰不存在的组合）
3. emoji 要和答案有清晰逻辑关系，不能完全无关

其他要求：
1. 答案用最常见的说法（如"西游记"而非"Journey to the West"，不要"复仇者联盟4：终局之战"这种带副标题的）
2. alternatives 给 1-3 个别名/简称/英文名（可为空数组）
3. category 只能是"成语"或"影视"
${avoid ? `4. 避免与这些已有题重复（emoji 和答案都不能撞）：${avoid}\n` : ""}请严格返回以下 JSON 格式（不要有任何其他文字）：
